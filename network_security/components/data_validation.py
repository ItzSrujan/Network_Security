...
class DataValidation:
    ...
    def check_numerical_columns(self, dataframe : pd.DataFrame) -> bool:
        try:
            num_of_numerical_cols = dataframe.select_dtypes(include = "int64").shape[1]
            logging.info(f"Required number of numerical columns : {num_of_numerical_cols}")
            logging.info(f"Data frame has numerical columns : {dataframe.select_dtypes(include = 'int64').shape[1]}")
            if dataframe.select_dtypes(include = "int64").shape[1] == num_of_numerical_cols:
                return True
            return False
        except Exception as e:
            raise NetworkSecurityException(e, sys)
    ...
    def initiate_data_validation(self) -> DataValidationArtifact:
        try:
            ...
            num_status = self.check_numerical_columns(dataframe = train_df)
            if not num_status:
                error_message = f"Train dataframe does not contain all numerical columns"
            num_status = self.check_numerical_columns(dataframe = test_df)
            if not num_status:
